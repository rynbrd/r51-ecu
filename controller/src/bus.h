...
class Node {
    public:
        ...
        virtual bool recieve(Frame* frame) = 0;
...
// Bus connects a series of nodes which send and recieve frames. Frames are
...
class Bus {
    public:
        Bus(Node** nodes, int count) : nodes_(nodes), count_(count) {}

        // Called on each main loop iteration. Calls recieve on each node and
        // broadcasts any recieved frames.
        void loop();
