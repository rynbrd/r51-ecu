...
class ClimateTest : public TestOnce {
    public:
        ...
        void enableClimate(Climate* climate) {
            CAN20Frame state54A(0x54A, 0, {0x3C, 0x3E, 0x7F, 0x80, 0x3C, 0x41, 0x00, 0x58});
            CAN20Frame state54B(0x54B, 0, {0x59, 0x8C, 0x05, 0x24, 0x00, 0x00, 0x00, 0x02});
...
testF(ClimateTest, Init) {
    ...
    CAN20Frame init540(0x540, 0, {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
    CAN20Frame init541(0x541, 0, {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
    CAN20Frame ready540(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00});
    CAN20Frame ready541(0x541, 0, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
...
testF(ClimateTest, TurnOff) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x00, 0x84, 0x00});
    assertYieldFrame(control, expect);

    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00});
    assertYieldFrame(control, expect);
}
...
testF(ClimateTest, ToggleAuto) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x00, 0x24, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, ToggleAc) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x08, 0x04, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, ToggleDual) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x00, 0x0C, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, CycleMode) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x00, 0x05, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, ToggleDefog) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, ToggleRecirculate) {
    ...
    expect = CAN20Frame(0x541, 0, {0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
    assertYieldFrame(control, expect);

    expect = CAN20Frame(0x541, 0, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
    assertYieldFrame(control, expect);
}
...
testF(ClimateTest, TriggerFanSpeedUp) {
    ...
    expect = CAN20Frame(0x541, 0, {0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, TriggerFanSpeedDown) {
    ...
    expect = CAN20Frame(0x541, 0, {0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, TriggerDriverTempWhenOn) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0xFF, 0x00, 0x20, 0x04, 0x00});
    assertYieldFrame(control, expect);
    ...
    // increase temp
    control = Event((uint8_t)SubSystem::CLIMATE, (uint8_t)ClimateEvent::INC_DRIVER_TEMP_CMD);
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x01, 0x00, 0x20, 0x04, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, TriggerPassengerTempWhenOn) {
    ...
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0xFF, 0x20, 0x04, 0x00});
    assertYieldFrame(control, expect);
    ...
    // increase temp
    control = Event((uint8_t)SubSystem::CLIMATE, (uint8_t)ClimateEvent::INC_PASSENGER_TEMP_CMD);
    expect = CAN20Frame(0x540, 0, {0x60, 0x40, 0x00, 0x00, 0x01, 0x20, 0x04, 0x00});
    assertYieldFrame(control, expect);
...
testF(ClimateTest, TickOffState) {
    ...
    CAN20Frame state54A(0x54A, 0, {0x3C, 0x3E, 0x7F, 0x80, 0x00, 0x00, 0x00, 0x2C});
    CAN20Frame state54B(0x54B, 0, {0xF2, 0x00, 0x00, 0x24, 0x00, 0x00, 0x00, 0x02});
