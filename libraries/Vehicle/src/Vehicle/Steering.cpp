...
void SteeringKeypad::emit(const Caster::Yield<Message>& yield) {
    sw_a_.update();
    if (sw_a_.onPress(0))  {
        key_.id((uint8_t)SteeringKey::POWER);
        key_.pressed(true);
        yield(key_);
    } else if (sw_a_.onPress(1)) {
        key_.id((uint8_t)SteeringKey::SEEK_DOWN);
        key_.pressed(true);
        yield(key_);
    } else if (sw_a_.onPress(2)) {
        key_.id((uint8_t)SteeringKey::VOLUME_DOWN);
        key_.pressed(true);
        yield(key_);
    ...
    sw_b_.update();
    if (sw_b_.onPress(0))  {
        key_.id((uint8_t)SteeringKey::MODE);
        key_.pressed(true);
        yield(key_);
    } else if (sw_b_.onPress(1)) {
        key_.id((uint8_t)SteeringKey::SEEK_UP);
        key_.pressed(true);
        yield(key_);
    } else if (sw_b_.onPress(2)) {
        key_.id((uint8_t)SteeringKey::VOLUME_UP);
        key_.pressed(true);
        yield(key_);
